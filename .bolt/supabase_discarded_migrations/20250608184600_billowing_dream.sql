...
DECLARE
  ...
  table_names text[] := ARRAY['users', 'user_roles', 'role_permissions', 'user_sessions', 
...
BEGIN
  ...
  FOR current_table_name IN SELECT unnest(table_names)
  LOOP
    IF EXISTS (
      SELECT FROM information_schema.tables 
      WHERE table_schema = 'public' AND table_name = current_table_name
    ) THEN
      EXECUTE format('ALTER TABLE %I ENABLE ROW LEVEL SECURITY', current_table_name);
    END IF;
  END LOOP;
END $$;
