...
export default function AIConfiguration() {
  ...
  const [activeTab, setActiveTab] = useState<'config' | 'images' | 'writing'>('config');
  ...
  // Writing Assistant Connection Testing
  ...
  const loadUsageStats = async () => {
    try {
      // Load individual provider stats (would come from database)
      ...
      // Calculate overall stats from individual providers
      const totalImagesGenerated = Object.values(mockProviderStats).reduce((sum, stats) => sum + stats.imagesGenerated, 0);
      const totalImageCost = Object.values(mockProviderStats).reduce((sum, stats) => sum + stats.totalCost, 0);
      const totalWritingCost = Object.values(mockWritingStats).reduce((sum, stats) => sum + stats.totalCost, 0);
      const totalDailyLimit = Object.values(mockProviderStats).reduce((sum, stats) => sum + stats.dailyLimit, 0);
      const totalMonthlyUsage = totalImageCost + totalWritingCost;

      setUsageStats({
        imagesGenerated: totalImagesGenerated,
        totalCost: totalImageCost + totalWritingCost,
  ...
  const testWritingConnection = async (provider: string) => {
    ...
    try {
      ...
      // Simulate API test to avoid CORS issues in development
      ...
      if (provider === 'openai-gpt') {
        ...
        isValidFormat = config.apiKey.startsWith('sk-') && config.apiKey.length >= 40;
      } else if (provider === 'anthropic-claude') {
        // Anthropic keys start with 'sk-ant-' 
        isValidFormat = config.apiKey.startsWith('sk-ant-') && config.apiKey.length >= 40;
      } else if (provider === 'google-gemini') {
        // Google API keys are typically 39 characters
        isValidFormat = config.apiKey.length >= 30 && !config.apiKey.includes(' ');
      }
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center space-x-3">
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg transition-colors flex items-center space-x-2 ${
                          writingConnectionStatus[provider] === 'success' 
                            ? 'bg-green-600 hover:bg-green-700' 
                            : writingConnectionStatus[provider] === 'error'
                            ? 'bg-red-600 hover:bg-red-700'
                            : 'bg-blue-600 hover:bg-blue-700'
                        } ${testingWritingConnection[provider] ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
                        {testingWritingConnection[provider] ? (
                          ...
                          <div title={writingConnectionErrors[provider] || 'Connection failed'}>
                            <AlertCircle className="h-4 w-4" />
                            <span>Failed</span>
                          </div>
                        ) : (
                      ...
                      {(writingConnectionStatus[provider] === 'success' || writingConnectionStatus[provider] === 'error') && (
                        <button
                          onClick={() => resetWritingConnection(provider)}
                          className="px-3 py-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white transition-colors flex items-center space-x-1"
                      ...
                      <label className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          checked={config.enabled}
                          onChange={(e) => setWritingConfigs(prev => ({
                            ...prev,
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setWritingConfigs(prev => ({
                          ...prev,
                          [provider]: { ...prev[provider], maxTokens: parseInt(e.target.value) || 2000 }
                        }))}
                        className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2"
                        min="100"
                        max="8000"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setWritingConfigs(prev => ({
                          ...prev,
                          [provider]: { ...prev[provider], temperature: parseFloat(e.target.value) }
                        }))}
                        className="w-full"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setWritingConfigs(prev => ({
                          ...prev,
                          [provider]: { ...prev[provider], maxRequestsPerDay: parseInt(e.target.value) || 500 }
                        }))}
                        className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2"
                        min="1"
                        max="10000"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setWritingConfigs(prev => ({
                          ...prev,
                          [provider]: { ...prev[provider], costPerRequest: parseFloat(e.target.value) || 0.03 }
                        }))}
                        className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2"
                        min="0.001"
                        max="1"
