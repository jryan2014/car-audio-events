-- Advertisement Image Management System Database Schema (Simple Version)
-- Run this in Supabase SQL Editor
-- This version uses VARCHAR IDs instead of UUIDs to avoid compatibility issues

-- Enable UUID extension if not already enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Advertisement Images Table (using VARCHAR for advertisement_id)
...
-- Advertisement A/B Tests Table (using VARCHAR for advertisement_id)
CREATE TABLE IF NOT EXISTS advertisement_ab_tests (
    ...
    -- Images being tested (using UUIDs for the image references)
...
-- Sample data for testing (using simple string IDs)
