...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (userError || !userData || userData.membership_type !== 'admin') {
      return new Response(
        JSON.stringify({ error: 'Only administrators can approve events' }),
        { 
          status: 403, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    // Call the approve_event function
    ...
    if (approvalError) {
      return new Response(
        JSON.stringify({ error: 'Failed to approve event', details: approvalError.message }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
