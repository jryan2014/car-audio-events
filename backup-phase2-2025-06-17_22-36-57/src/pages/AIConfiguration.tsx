...
export default function AIConfiguration() {
  ...
  const getConnectionStatus = (provider: string, type: 'image' | 'writing') => {
    ...
    const configMap = type === 'image' ? configs : writingConfigs;
    const config = configMap[provider];
    const status = statusMap[provider];
    
    // If no API key is configured, show "Not Configured"
    if (!config?.apiKey || config.apiKey.trim() === '') {
      return { status: 'not-configured', color: 'gray', text: 'Not Configured' };
    }
    
    // If API key exists but no status check has been done yet
    if (!status) {
      return { status: 'unknown', color: 'gray', text: 'Checking...' };
    }
  ...
  const loadUsageStats = async () => {
    try {
      ...
      // Fallback to empty stats if database fails - no more hardcoded values
      console.warn('⚠️ Database unavailable, showing empty stats until connection restored');
      
      setProviderStats({});
      setWritingProviderStats({});
      setUsageStats({ imagesGenerated: 0, totalCost: 0, dailyLimit: 0, monthlyUsage: 0 });
    }
  };
            ...
            {Object.entries(configs).map(([provider, config]) => (
              <div key={provider} className="bg-gray-800 border border-gray-700 rounded-xl p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center space-x-3">
                    {/* Connection Status Display - Always show */}
                    {(() => {
                        ...
                        <div className={`px-3 py-2 rounded-lg flex items-center space-x-2 ${
                          ...
                          {status.color === 'green' ? (
                            <>
                              ...
                              <span className="text-green-400 text-sm">{status.text}</span>
                            </>
                          ) : status.color === 'red' ? (
                            <>
                              <AlertCircle className="h-4 w-4 text-red-400" />
                              <span className="text-red-400 text-sm">{status.text}</span>
                            </>
                          ) : status.color === 'yellow' ? (
                            <>
                              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-yellow-400"></div>
                              <span className="text-yellow-400 text-sm">{status.text}</span>
                            </>
                          ) : (
                            <>
                              <AlertCircle className="h-4 w-4 text-gray-400" />
                              <span className="text-gray-400 text-sm">{status.text}</span>
                            </>
                          )}
            ...
            <div className="bg-gray-700/50 border border-gray-600 rounded-lg p-6">
              ...
              <div className="mt-6">
                <label className="block text-sm font-medium mb-3 flex items-center space-x-2">
                  <span>Text Overlay Configuration</span>
                  <div title="Configure how text overlays will be applied to generated images">
                    <AlertCircle className="h-4 w-4 text-blue-400" />
                  </div>
                </label>
                <div className="bg-gray-600/30 rounded-lg p-6">
                  <p className="text-sm text-gray-300 mb-6">
                    Images will be generated text-free with designated areas for overlays. Configure overlay zones and styling options for ad management.
                  </p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    <div>
                      <h5 className="font-medium text-white mb-3 flex items-center space-x-2">
                        <span>Overlay Zones</span>
                        <div title="Define areas where text will be overlaid">
                          <HelpCircle className="h-4 w-4 text-blue-400" />
                        </div>
                      </h5>
                      <div className="space-y-3">
                        {[
                          { id: 'header', name: 'Header/Message', desc: 'Main headline or message' },
                          { id: 'cta', name: 'Call-to-Action', desc: 'Action button or CTA text' },
                          { id: 'contact', name: 'Contact Info', desc: 'Phone, website, or address' },
                          { id: 'product', name: 'Product Details', desc: 'Product name or specifications' },
                          { id: 'offer', name: 'Special Offers', desc: 'Pricing or promotional text' },
                          { id: 'logo', name: 'Logo Placement', desc: 'Brand logo positioning' }
                        ].map((zone) => (
                          <div key={zone.id} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                            <div>
                    ...
                    <div>
                      <h5 className="font-medium text-white mb-3 flex items-center space-x-2">
                        <span>Overlay Styling</span>
                        <div title="Configure text overlay appearance">
                          <HelpCircle className="h-4 w-4 text-blue-400" />
                        </div>
                      </h5>
                      <div className="space-y-4">
                        {/* Text Style */}
                        <div>
                          <label className="block text-sm font-medium mb-2">Default Text Style</label>
                        ...
                        {/* Background Style */}
                        <div>
                          <label className="block text-sm font-medium mb-2">Background Treatment</label>
                          <select className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm">
                        ...
                        {/* Shadow/Outline */}
                        <div>
                          <label className="block text-sm font-medium mb-2">Text Enhancement</label>
                          <div className="space-y-2">
                            <label className="flex items-center space-x-2">
                              <input type="checkbox" className="rounded border-gray-600 bg-gray-700 text-electric-500" />
                        ...
                        {/* Responsive Sizing */}
                        <div>
                          <label className="block text-sm font-medium mb-2">Responsive Sizing</label>
                          <label className="flex items-center space-x-2">
                            <input type="checkbox" defaultChecked className="rounded border-gray-600 bg-gray-700 text-electric-500" />
                            <span className="text-sm">Auto-adjust text size for different banner dimensions</span>
                          </label>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="border-t border-gray-600 pt-4">
                    <h5 className="font-medium text-white mb-3">Overlay Preview</h5>
                    <div className="bg-gray-700/30 rounded-lg p-4 text-center">
                      <div className="text-sm text-gray-400 mb-2">
                        Preview will show how text overlays appear on generated images
                      </div>
                      <button className="px-4 py-2 bg-electric-500 hover:bg-electric-600 text-white rounded-lg text-sm transition-colors">
                        Generate Preview
                      </button>
                    </div>
                  </div>
              ...
              <div className="mt-6">
                <label className="block text-sm font-medium mb-3 flex items-center space-x-2">
                  ...
                  <div title="Fine-tune image generation parameters">
                    <AlertCircle className="h-4 w-4 text-blue-400" />
                  </div>
                </label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2">
                      ...
                      <option value="hd">HD Quality (+cost)</option>
                      <option value="ultra">Ultra HD (+higher cost)</option>
                    </select>
                  </div>
                  <div>
              ...
              <div className="mt-6">
                <div className="bg-blue-900/20 border border-blue-700/50 rounded-lg p-4">
                  <h5 className="font-medium text-white mb-2 flex items-center space-x-2">
                    ...
                    <span>Clear Space Guidelines</span>
                  </h5>
                  <div className="text-sm text-gray-300 space-y-1">
                    <p>• Images will maintain clear zones for text overlays</p>
                    <p>• Important visual elements will avoid designated text areas</p>
                    <p>• Background contrast will be optimized for text readability</p>
                    <p>• Logo placement areas will remain uncluttered</p>
                  </div>
                </div>
              </div>

              {/* Legacy Text Zone Reference */}
              <div className="mt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-700/30 rounded-lg p-4">
                    <h5 className="font-medium text-white mb-2">Standard Text Zones:</h5>
