-- Fix rules_templates table for OrganizationManager (Version 3)
-- This handles existing NULL values and table conflicts properly

-- First, let's check if the table exists and what columns it has
DO $$
BEGIN
    -- Check if rules_templates table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'rules_templates') THEN
        RAISE NOTICE 'rules_templates table already exists, checking columns...';
    ...
    ELSE
        ...
        RAISE NOTICE 'Created rules_templates table from scratch';
    END IF;
END $$;

-- Fix any NULL values in existing data before setting NOT NULL constraint
UPDATE public.rules_templates 
SET rules_content = 'Default rules content - please update' 
...
ALTER TABLE public.rules_templates ALTER COLUMN rules_content SET NOT NULL;
ALTER TABLE public.rules_templates ALTER COLUMN name SET NOT NULL;
...
DROP POLICY IF EXISTS "Allow authenticated users to read rules templates" ON public.rules_templates;
DROP POLICY IF EXISTS "Allow authenticated users to insert rules templates" ON public.rules_templates;
DROP POLICY IF EXISTS "Allow authenticated users to update rules templates" ON public.rules_templates;
DROP POLICY IF EXISTS "Allow authenticated users to delete rules templates" ON public.rules_templates;
...
-- Clear existing data to avoid conflicts, then insert sample data
DELETE FROM public.rules_templates;
...
-- Add/recreate trigger to update updated_at timestamp
DROP TRIGGER IF EXISTS trigger_rules_templates_updated_at ON public.rules_templates;
DROP FUNCTION IF EXISTS update_rules_templates_updated_at();
...
-- Success message
SELECT 'Rules templates table fixed successfully with sample data' as result; 