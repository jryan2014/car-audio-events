-- NAVIGATION SYSTEM SECURITY POLICIES (SIMPLIFIED)
...
CREATE POLICY "navigation_templates_admin_full" ON navigation_templates
    FOR ALL USING (
        EXISTS (
            ...
            AND (auth.users.raw_user_meta_data->>'membershipType')::text = 'admin'
        )
    );
...
-- Allow organization users to manage their own navigation context
CREATE POLICY "navigation_menu_items_org_manage" ON navigation_menu_items
    FOR ALL USING (
        ...
        EXISTS (
            SELECT 1 FROM auth.users 
            WHERE auth.users.id = auth.uid() 
            AND (auth.users.raw_user_meta_data->>'membershipType')::text = 'organization'
        )
    );
...
CREATE POLICY "navigation_menu_items_manufacturer_manage" ON navigation_menu_items
    FOR ALL USING (
        ...
        EXISTS (
            SELECT 1 FROM auth.users 
            WHERE auth.users.id = auth.uid() 
            AND (auth.users.raw_user_meta_data->>'membershipType')::text = 'manufacturer'
        )
    );
...
CREATE POLICY "navigation_menu_items_retailer_manage" ON navigation_menu_items
    FOR ALL USING (
        ...
        EXISTS (
            SELECT 1 FROM auth.users 
            WHERE auth.users.id = auth.uid() 
            AND (auth.users.raw_user_meta_data->>'membershipType')::text = 'retailer'
        )
    );
