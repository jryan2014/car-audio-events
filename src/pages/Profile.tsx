...
import MemberProfilePrivacySettings from '../components/MemberProfilePrivacySettings';
...
export default function Profile() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      await Promise.all([
        // Audio systems functionality moved to member-profile-settings
              ...
              <Accordion
                items={[
                  ...
                  },
                  {
                    ...
                    content: <MemberProfilePrivacySettings />
                  },
                  {
                    id: 'security',
                    title: 'Security',
                    ...
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-700/50 rounded-lg border border-gray-600">
                      <div className="flex items-center space-x-4">
                        <div className="p-2 bg-electric-500/20 rounded-lg">
                          <Monitor className="h-5 w-5 text-electric-500" />
                        </div>
                        <div>
                          <p className="text-white font-medium">{session.device || 'Unknown Device'}</p>
                          <p className="text-gray-400 text-sm">{session.location || 'Unknown Location'}</p>
                          <p className="text-gray-500 text-xs">Last active: {session.lastActive || 'Unknown'}</p>
                        </div>
                      </div>
                      {session.isCurrent && (
