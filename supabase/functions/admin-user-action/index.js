...
Deno.serve(function (req) { return __awaiter(void 0, void 0, void 0, function () {
    var supabaseClient, authHeader, token, _a, user, authError, _b, userData, userError, _c, userId, action, updateData, auditAction, updateError, auditError, error_1;
    var _d, _e;
        ...
        switch (_f.label) {
            ...
            case 3:
                ...
                if (userError || !userData || userData.membership_type !== 'admin' || userData.status !== 'active') {
                    return [2 /*return*/, new Response(JSON.stringify({ error: 'Insufficient permissions' }), {
                            status: 403,
                            headers: __assign(__assign({}, corsHeaders), { 'Content-Type': 'application/json' })
                        })];
                }
            ...
            case 4:
                _c = _f.sent(), userId = _c.userId, action = _c.action;
                if (!userId || !action) {
                    return [2 /*return*/, new Response(JSON.stringify({ error: 'Missing userId or action' }), {
                            status: 400,
                            headers: __assign(__assign({}, corsHeaders), { 'Content-Type': 'application/json' })
                        })];
                }
                ...
                switch (action) {
                    ...
                    case 'delete':
                        ...
                        break;
                    default:
                        return [2 /*return*/, new Response(JSON.stringify({ error: 'Invalid action' }), {
                                status: 400,
                                headers: __assign(__assign({}, corsHeaders), { 'Content-Type': 'application/json' })
                            })];
                }
            ...
            case 5:
                ...
                if (updateError) {
                    ...
                    return [2 /*return*/, new Response(JSON.stringify({ error: 'Failed to update user' }), {
                            status: 500,
                            headers: __assign(__assign({}, corsHeaders), { 'Content-Type': 'application/json' })
                        })];
                }
                return [4 /*yield*/, supabaseClient
                        ...
                        admin_id: user.id,
                        action: auditAction,
                        details: { userId: userId, action: action, timestamp: new Date().toISOString() }
                    })];
            case 6:
                auditError = (_f.sent()).error;
                if (auditError) {
                    console.error('Error logging audit action:', auditError);
                    // Don't fail the request if audit logging fails
                }
                return [2 /*return*/, new Response(JSON.stringify({ success: true, message: "User ".concat(action, " successful") }), {
                        headers: __assign(__assign({}, corsHeaders), { 'Content-Type': 'application/json' })
                    })];
            case 7:
                error_1 = _f.sent();
