...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`Geocoding address: ${eventData.address}, ${eventData.city}, ${eventData.state}`);
    ...
    if (geocodeResult) {
      ...
      if (fallbackCoords) {
        ...
        // Last resort - use state coordinates
        const stateCode = eventData.state.toUpperCase();
        const stateCoordinates: Record<string, [number, number]> = {
          'AL': [32.7794, -86.8287],
          'AK': [64.0685, -152.2782],
          'AZ': [34.2744, -111.6602],
    ...
    // If we have geocode results, create an event_location record
    if (event) {
      const locationData = {
        event_id: event.id,
        ...
        formatted_address: geocodeResult ? geocodeResult.formatted_address : `${eventData.city}, ${eventData.state}, ${eventData.country || 'US'}`,
      ...
      try {
        const { data: forceUpdateResult, error: forceUpdateError } = await supabaseClient
          .rpc('force_update_event_coordinates', { event_uuid: event.id });
