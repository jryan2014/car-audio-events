...
serve(async (req) => {
  ...
  try {
    ...
    const emailText = `Welcome to Car Audio Events, ${firstName}!
    
Thank you for joining our community. We're excited to have you on board!

You can now access your dashboard at: https://caraudioevents.com/dashboard
...
The Car Audio Events Team`;
    ...
    if (!emailResult.success) {
      throw new Error(`Failed to send welcome email: ${emailResult.error || 'Unknown error'}`);
    }
    
    console.log(`Welcome email sent via ${emailResult.provider} to ${to}`);
